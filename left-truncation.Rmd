...
date: Supplement--Analysis options for left truncation
output: 
  html_document:
    number_sections: no
    theme: flatly
...
```{r, out.width="60%", fig.cap="Downward view of aerial survey, with distance bands eminating from the transect.  Note area directly underneath plane where detections are not possible.", echo=FALSE}
...
pintail <- data.frame(Sample.Label=1,
                      Effort = L,
                      distance = pindist)
pin.df <- ds(pintail, convert.units = convert, adjustment="herm",
             cutpoints = seq(0, 12))
...
text(5.2,0.9, "pin.df <- ds(pintail, convert.units = convert, adjustment='herm',\ncutpoints = seq(0, 12))", adj=0, cex=0.8)

pin.left <- ds(pintail, convert.units = convert, adjustment = "herm",
...
text(6.1,1, paste("pin.left <- ds(pintail, convert.units = convert, \nadjustment = 'herm',\ntruncation=list(left=3, right=12), cutpoints=seq(3,12))"),
     adj=0, cex=0.8)
...
pin.subtract <- ds(pintail.adjusted, convert.units=convert, adjust="herm",
                   cutpoints=seq(0,9))
...
text(2.1,1, paste("pintail.subtract <- pintail$distance - 3",
                ...
                "pin.subtract <- ds(pintail.adjusted, convert.units=convert, \nadjust='herm',cutpoints=seq(0,9))",
              sep="\n"), adj=0, cex=0.8)
```

## References

Alldredge, J.R., and C.E. Gates. (1985) Line transect estimators for left-truncated distributions. Biometrics 41:273–280. https://doi.org/10.2307/2530663.

Buckland, S.T., D.R. Anderson, K.P. Burnham, J.L. Laake, D.L. Borchers, and L. Thomas. (2001) Introduction to Distance Sampling: Estimating Abundance of Biological Populations. Oxford, New York: Oxford University Press.
